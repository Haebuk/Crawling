...
with BrownstreetScrapping() as brownstreet:
    ...
    base_url, url_list = brownstreet.get_shop_page()
    for n in url_list:
        for i in range(1, 200):
            if soldout:
                ...
                break
            try:
                brownstreet.land_first_page(url=base_url+str(n), i=i)
            except:
                print(e, '페이지 없음')
                break
            product_list = brownstreet.get_product_list()
            for iter in range(len(product_list)):
                total_list = data.load_json()
                print('----------------{} page, {} -----------------'.format(i, iter+1))

                try:
                    product_category_name = brownstreet.get_product_category()
                    product_price = brownstreet.get_product_price(iter)
                    product_link = brownstreet.click_product(iter)
                    is_sold_out = brownstreet.is_product_sold_out()
                    if is_sold_out:
                        soldout = True
                        break
                    product_thumbnail = brownstreet.get_product_thumbnail()
                    product_name = brownstreet.get_product_name()
                    product_brand_before = brownstreet.get_product_brand(product_name)
